...
const AreaFilter: React.FC = () => {
  ...
  return (
    <div className="mb-6">
      <h2 className="text-base font-medium mb-3 flex items-center text-netflix-accent">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm transition-colors flex items-center ${
              selectedArea === null
                ? 'bg-netflix-accent text-black shadow-md font-semibold'
                : 'bg-netflix-card text-netflix-text hover:bg-netflix-cardHover'
            }`}
          >
            <Scale size={14} className="mr-1" />
            <span>Todas</span>
            <span className="ml-1 bg-black/20 rounded-full w-5 h-5 flex items-center justify-center text-xs">
              {books.length}
            </span>
          </button>
